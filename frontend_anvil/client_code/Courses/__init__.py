...
class Courses(CoursesTemplate):
    ...
    #     for item in items:
    #         exists = False
    #         for comp in self.content_panel.get_components():
    #             if isinstance(comp, anvil.CheckBox) and comp.text == item:
    #                 exists = True
    #                 break
    #         if not exists:
    #             if not str(item).isdigit():
    #                 checkbox = anvil.CheckBox(text=item)
    #                 checkbox.set_event_handler("change", self.check_box_changed)
    #                 self.content_panel.add_component(checkbox)
    #             else:
    #                 sem_num = Label(text=item, align='center')
    #                 self.content_panel.add_component(sem_num)
    ...
    #     checked = False
    #     for checkbox in self.content_panel.get_components():
    #         if isinstance(checkbox, CheckBox) and checkbox.checked:
    #             # Append the value of the checked checkbox to the list
    #             self.checked_values.add(checkbox.text)
    #             checked = True
    
    #     # Set the visibility of the button based on whether a checkbox is checked
    #     self.generate.visible = checked              
...
#         self.generate.remove_from_parent()
    ...
    def load_routine(self):
          ...
          unique_checked_values = list(self.checked_values)
          dj_day, dj_course, dj_msg, final_routine, comparison_list, final_time_list, overlapped_courses_1, overlapped_courses_2 = anvil.server.call('generate_default_routine', unique_checked_values)
          # print(overlapped_courses_2)
          # General Routine
          dj_rout = []
          for i in range(len(dj_day)):
              dj_rout.append((dj_day[i], dj_course[i]))
              
          days, courses = [], []
          for day, course in dj_rout:
              days.append(day)
              for i in range(len(course)):
                  courses.append(course[i])
              # Add the day labels and course labels to the routine panel
              for day in days:
                  ...
                  day_label = Label(text=day, width='80px', align="left", font_size="3")
                  day_panel.add_component(day_label)
                      
                  # Add a panel with a border for each course inline with the day label to the day panel
                  for course in courses:
                      course_panel = ColumnPanel(border="1px solid black", width="100px")
                      label = Label(text=course, align="center", font_size="3")
                      course_panel.add_component(label)
                      day_panel.add_component(course_panel)
                  self.content_panel.add_component(day_panel)
              days, courses = [], []
          ...
          for key, val in dj_msg.items():
              if key in dj_day and dj_msg[key] != '':
                  ...
                  # final_msg = f'{key}: You have {stripped_msg} overlaps'
          ...
          if len(err_panel.get_components()) == 0:
              ...
              self.content_panel.add_component(h_label)
              self.content_panel.add_component(healthy_label)
          # else add the err_panel to reports section
          else:
              h_label = Label(text='Overlaps', align='left', font_size="5")
              self.content_panel.add_component(h_label)
              self.content_panel.add_component(err_panel)
              fix_button = Button(text='Fix Routine')
              self.content_panel.add_component(fix_button)
              fix_button.set_event_handler('click', self.fix_button_click)
    ...
    #     # Add the routine panel to the content panel
    #     self.content_panel.clear()
    #     self.content_panel.add_component(routine_panel)
...
# #         alert('I will fix you...')
#         # Access the instance variable without passing it as a parameter
#         # final_routine = self.final_routine
#         self.content_panel.clear()
...
#         self.content_panel.add_component(model_1_label)
#         self.get_updated_routine(trial_routine_1, self.content_panel)
...
#         self.content_panel.add_component(model_2_label)
#         self.get_updated_routine(trial_routine_2, self.content_panel)
...
#                 crs_to_append = v[i][0] + ' ' + v[i][1]
#                 u_courses.append(crs_to_append)
#             # Add the day labels and course labels to the routine panel
#             for day in u_days:
# #                 print(day)
#                 # Create a new FlowPanel for the day
...
#             u_days, u_courses = [], []
# #             self.add_component(updated_data_row_panel)
